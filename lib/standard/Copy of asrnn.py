...
class ASRNN(nn.Module):
    def __init__(self,hp,pretrained_embeddings=None):
        ...
        self.embeddings=nn.Embedding(hp.n_mels+1,hp.embedding_dim)
        self.pos_embeddings=nn.Embedding(hp.n_mels+1,hp.n_mels)
    ...
    def encoder(self,x,input_lens,SpecAugment=False):
        # print(x.shape)
        # tau=x['tau']
        if SpecAugment:
    ...
    def decoder(self,y):
        ...
        return y
    def forward(self,x,input_lens,SpecAugment=True,lm=None):#,OutAugment=0.0):
        x=x.squeeze()
        # x=self.embeddings(x.round().int())
        # x=torch.mean(x,-2)
        pos=torch.arange(1,self.hp.n_mels+1).to(self.hp.device)#.unsqueeze(0)
        # print(pos.shape)
        # pos=pos.unsqueeze(-1).repeat(self.hp.batch_size,1,self.hp.max_len)#.unsqueeze(1)
        pos=pos.unsqueeze(-1).repeat(self.hp.batch_size,1,1)#.unsqueeze(1)
        pos=self.pos_embeddings(pos.squeeze())
        ...
        # print(tau.shape)
        x,output_lens=self.encoder(x,input_lens,SpecAugment)
        # print(x.shape)
        # print(x)
        # y=self.decoder(x)
