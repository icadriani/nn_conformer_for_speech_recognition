...
class ASRNN(nn.Module):
    def __init__(self,hp,pretrained_embeddings=None):
        ...
        # self.standard_linear=nn.Linear(self.conv_sub_sampling.out_size,hp.standard_linear_nodes)
    ...
    def time_warping(self,x,tau):
        ...
        for u in range(x.shape[0]):
            ...
            if tau[u]<2*self.hp.warping_param_W:
                ...
                w0=random.randint(self.hp.warping_param_W,tau[u]-self.hp.warping_param_W)
            for t in range(tau[u]):
                if t<=w0:
                    wt=int(((w0+w)/w0)*t)
                else:
        ...
        for b in range(x.shape[0]):
            ...
            for f in range(x.shape[1]):
                # print(Wt[b])
                xw=x[b][f][Wt[b]].cpu().numpy().tolist()
                x_warp[b].append(xw)
        # print(x_warp)
